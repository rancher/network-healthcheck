	...
	for i := 1; i <= pingCount; i++ {
		...
		ping.OnRecv = func(ip *net.IPAddr, d time.Duration) {
			if ip.String() == p.DestIP {
				log.Debugf("Received a ping reply from %s, %d/%d", p.DestIP, i, pingCount)
				receivedCount++
			}
		}
		log.Debugf("Do a ping check to %s, %d/%d", p.DestIP, i, pingCount)
		err := ping.Run()
		if err != nil {
			log.Errorf("Failed to ping %s, %v", p.DestIP, err)
		}
	}
	if receivedCount < pingCount {
		log.Errorf("Lose ping data from %s", p.DestIP)
		p.Reachable = false
	} else {
		p.Reachable = true
	}
...
func (p *Peer) arpingCheck() {
	...
	for i := 1; i <= pingCount; i++ {
		log.Debugf("Do an arping check to %s, %d/%d", p.DestIP, i, pingCount)
		hwAddr, _, err := arping.Ping(net.ParseIP(p.DestIP))
		if err != nil {
			log.Errorf("Failed to arping %s, %v", p.DestIP, err)
			continue
		}
		if hwAddr.String() != "" {
			result[hwAddr.String()] = true
		}
		log.Debugf("Received an arping reply from %s, mac: %s, %d/%d", p.DestIP, hwAddr.String(), i, pingCount)
		time.Sleep(commonTimeout)
	}
	if len(result) > 1 {
		log.Errorf("Get multiple MAC from %s", p.DestIP)
		p.Reachable = false
	} else {
		p.Reachable = true
	}
...
func (p *Peer) httpCheck() {
	if p.IsRouter {
		log.Debugf("Do a http check for %s", p.DestIP)
		resp, err := http.Get(fmt.Sprintf("http://%s:8111/ping", p.DestIP))
		if err != nil || resp.StatusCode != http.StatusOK {
			log.Errorf("Router container %s is unreachable", p.DestIP)
			p.Reachable = false
		} else {
			p.Reachable = true
		}
		...
		log.Debugf("Finish a http check for %s", p.DestIP)
	}
}
...
func (s *Server) RunLoop() error {
	for {
		log.Debugf("Start main loop checking...")
		existContainers, err := s.calculatePeers()
		if err != nil {
			return err
		}
		s.checkPeers(existContainers)
		log.Debugf("Sleep main loop checking...wait...")
...
func (s *Server) RunRetryLoop() error {
	for {
		time.Sleep(checkInterval)
		log.Debugf("Start retry loop checking...")
		if getSyncMapLength(s.unreachablePeers) == 0 {
			log.Debugf("Nothing to do in retry loop...")
			continue
		}
		...
		log.Debugf("Sleep retry loop checking...wait...")
	}
}
